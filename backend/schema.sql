...
CREATE INDEX IF NOT EXISTS idx_concept_user_created ON concept_queries (user_id, created_at);
CREATE INDEX IF NOT EXISTS idx_concept_user_topic ON concept_queries (user_id, topic);
CREATE INDEX IF NOT EXISTS idx_concept_user_level ON concept_queries (user_id, level);
...
CREATE INDEX IF NOT EXISTS idx_explanations_favorite ON explanations (favorite);
...
CREATE INDEX IF NOT EXISTS idx_search_user_created ON search_history (user_id, created_at);
CREATE INDEX IF NOT EXISTS idx_search_domain ON search_history (detected_domain);
...
CREATE INDEX IF NOT EXISTS idx_conversation_user_created ON conversation_history (user_id, created_at);
CREATE INDEX IF NOT EXISTS idx_conversation_user_topic ON conversation_history (user_id, topic);
...
CREATE INDEX IF NOT EXISTS idx_topic_tracking_user ON topic_tracking (user_id);
...
ON CONFLICT (topic, domain) DO UPDATE SET summary = EXCLUDED.summary, keywords = EXCLUDED.keywords;
...
ON CONFLICT (query_id) DO UPDATE SET content = EXCLUDED.content, favorite = EXCLUDED.favorite;

INSERT INTO favorites (id, user_id, explanation_id, created_at)
VALUES
    (1, 1, 1, NOW())
ON CONFLICT (user_id, explanation_id) DO UPDATE SET created_at = EXCLUDED.created_at;
...
ON CONFLICT (user_id, normalized_topic) DO UPDATE SET frequency = EXCLUDED.frequency, current_level = EXCLUDED.current_level;
...
VALUES
    (1, 2, 2, 0, 0, 0, NOW())
ON CONFLICT (user_id) DO UPDATE SET total_interactions = EXCLUDED.total_interactions, updated_at = EXCLUDED.updated_at;
